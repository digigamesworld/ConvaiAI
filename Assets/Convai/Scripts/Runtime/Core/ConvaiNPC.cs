...
namespace Convai.Scripts.Runtime.Core
{
    ...
    public class ConvaiNPC : MonoBehaviour
    {
        ...
        private async void Start()
        {
            ...
            // Step 1: Define custom DNS resolver
            var channelOptions = new ChannelOption[]
            {
            new ChannelOption(ChannelOptions.PrimaryUserAgent, "Custom/1.0")
            };

            // Step 2: Create SSL credentials for secure communication
            SslCredentials credentials = new SslCredentials();

            // Step 3: Set up the custom DNS resolver using the custom DNS server
            var channel = GrpcChannel.ForAddress(GRPC_API_ENDPOINT, new GrpcChannelOptions
            {
                Credentials = credentials,
                MaxReceiveMessageSize = 10 * 1024 * 1024, // Example size; adjust as needed
                                                          // Add custom DNS settings here if gRPC allows configuring resolvers directly
                                                          // GrpcClientOptions.DnsResolvers (as of now, .NET gRPC doesn’t support it directly)
            });

            // Step 4: Initialize the gRPC client for Convai
            _client = new ConvaiService.ConvaiServiceClient(channel);

            // Test the connection
            try
            {
                var response = await _client.GetResponseAsync(new GetResponseRequest { /* Your request params */ });
